...
// production (see SignInApp.tsx for why this doesn't live there).
import "@mfe/design-system/tailwind.css";
import { HelperNote } from "@mfe/design-system";
import { SignInApp } from "./SignInApp";

// SignInApp calls useNavigate()/useSearchParams(), so standalone mode
// needs its own <BrowserRouter> too — in production this comes from the
// Shell's single <BrowserRouter> instead (see apps/shell/src/bootstrap.tsx).
//
    ...
    <BrowserRouter>
      <div style={{ maxWidth: 420, margin: "24px auto", padding: "0 16px" }}>
        <HelperNote>
          ...
          available; every <code>HelperNote</code> renders unconditionally instead. The login
          request below only succeeds if this dev server can reach the Ship Server directly.
        </HelperNote>
